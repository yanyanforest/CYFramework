...
+(void)bridge:(UITableView *)tv dsd:(CYUITVDSDBase *)dsd{
    if (tv) {
        tv.delegate = dsd;
        tv.dataSource = dsd;
        dsd.tableView = tv;
...
+(void)updataView:(UITableView *)tv dsd:(CYUITVDSDBase *)dsd{
    ...
    [dsd.classesIdbyReusedCells enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL * stop) {
        ...
        [tv registerClass: cls forCellReuseIdentifier:dsd.reusedCellIdentifier];
